...
export const PopulationPyramid = ({ data, width = 800, height = 600, margin = { top: 40, right: 60, bottom: 60, left: 60 }, maleColor = '#2196f3', femaleColor = '#f50057', title = 'Population Pyramid' }) => {
    ...
    useEffect(() => {
        ...
        const maleAxis = axisBottom(scaleLinear()
            .domain([maxValue, 0])
            .range([0, innerWidth / 2])).ticks(5);
        g.append('g')
            .attr('transform', `translate(0,${innerHeight})`)
            .call(maleAxis)
            .style('font-size', '10px');
        // Add X axis labels for females (right side)
        const femaleAxis = axisBottom(scaleLinear()
            .domain([0, maxValue])
            .range([0, innerWidth / 2])).ticks(5);
        g.append('g')
    ...
    return (<Box sx={{ width: '100%', overflow: 'auto', display: 'flex', justifyContent: 'center' }}>
