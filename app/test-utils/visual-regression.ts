...
type VisualTestFixtures = {
  visualTestPage: Page;
  takeScreenshot: (name: string, options?: { fullPage?: boolean }) => Promise<void>;
...
export const test = base.extend<VisualTestFixtures>({
  ...
  takeScreenshot: async ({ page }, use) => {
    const takeScreenshotFn = async (name: string, options: { fullPage?: boolean } = {}) => {
  ...
  compareScreenshots: async ({ page }, use) => {
    const compareScreenshotsFn = async (baseline: string, current: string): Promise<boolean> => {
 ...
 */
export async function navigateAndStabilize(page: Page, url: string): Promise<void> {
  await page.goto(url);
  await page.waitForLoadState('networkidle');
...
export async function screenshotComponent(
  ...
  const { viewport = VIEWPORTS.desktop, darkMode = false, rtl = false } = options;
...
export async function testResponsiveDesign(
  ...
  const { viewports = ['mobile', 'tablet', 'desktop'], darkMode, rtl } = options;
