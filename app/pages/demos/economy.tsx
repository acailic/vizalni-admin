        ...
        <Box sx={{ mt: 2 }}>
          {economicIndicators
            ...
            .map(d => (
              <Chip
                key={d.year}
                label={`${d.year}: ${d.event}`}
                size="small"
                sx={{ mr: 1, mb: 1 }}
                color={d.gdpGrowth < 0 ? 'error' : 'primary'}
                variant="outlined"
              />
            ))
          }
  ...
  return (
    <DemoPageTemplate
      ...
      columns={[
        ...
        { key: 'inflation', header: locale === 'sr' ? 'Inflacija (%)' : 'Inflation (%)', width: 150 },
        { key: 'unemployment', header: locale === 'sr' ? 'Nezaposlenost (%)' : 'Unemployment (%)', width: 150 },
        { key: 'event', header: locale === 'sr' ? 'Događaj' : 'Event', width: 250 },
      ]}
    />
  );
}
